...
const doctorImageSecondary = "/manus-storage/yuliia-standing-option-balanced-head_102214b0.png";
const doctorImageSecondaryRetouched = "/manus-storage/yuliia-soft-under-eye-option_e2c28d7e.png";
...
export default function Home() {
  ...
  return (
    <main className="site-shell">
      ...
      <section id="team" className="team-section section-pad"><div className="team-photo"><img src={doctorImage} alt="Мезінова Аліна Віталіївна — лікар-стоматолог Active Medical" /><div className="team-photo-secondary team-photo-yuliia-pair"><img src={doctorImageSecondary} alt="Диченко Юлія Андріївна — портрет у білій медичній формі" /><img src={doctorImageSecondaryRetouched} alt="Диченко Юлія Андріївна — портрет із делікатною корекцією тіні під очима" /></div><div className="team-photo-tertiary"><img src={doctorImageTertiary} alt="Погулич Ярослав Євгенович — імплантолог, хірург, ортопед Active Medical" /></div><div className="team-photo-quaternary"><img src={doctorImageQuaternary} alt="Федоров Іван Михайлович — лікар-стоматолог Active Medical" /></div></div><div className="team-copy"><div className="section-kicker">06 / Команда</div><h2>Ваші лікарі —<br /><em>ваші союзники.</em></h2><p>Ми не ховаємося за білими халатами. Говоримо просто, працюємо уважно і завжди залишаємо вам право на запитання.</p><div className="doctor-list">{doctors.map((doctor, index) => <div className="doctor-row" key={doctor.name}><span className="doctor-number">0{index + 1}</span><div><strong>{doctor.name}</strong><span>{doctor.role} · {doctor.detail}</span></div><ArrowUpRight size={17} /></div>)}</div></div></section>
