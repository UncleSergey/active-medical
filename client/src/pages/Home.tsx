  ...
  return (
    <main className="site-shell">
      ...
      <section id="top" className="hero-section">
        ...
        <div className="hero-visual"><img src={heroImage} alt="Світлий кабінет стоматологічної клініки" /><div className="hero-caption"><span>Миколаїв · ЖК Рів'єра</span><span>Пн–Сб / 09:00–20:00</span></div><div className="hero-stamp">AM<br /><small>care first</small></div></div>
      </section>
      ...
      <section id="team" className="team-section section-pad"><div className="team-photo"><img src={doctorImage} alt="Лікарка стоматологиня Active Medical" /><div className="photo-label">Люди<br /><em>поруч</em></div></div><div className="team-copy"><div className="section-kicker">04 / Команда</div><h2>Ваші лікарі —<br /><em>ваші союзники.</em></h2><p>Ми не ховаємося за білими халатами. Говоримо просто, працюємо уважно і завжди залишаємо вам право на запитання.</p><div className="doctor-list">{doctors.map((doctor, index) => <div className="doctor-row" key={doctor.name}><span className="doctor-number">0{index + 1}</span><div><strong>{doctor.name}</strong><span>{doctor.role} · {doctor.detail}</span></div><ArrowUpRight size={17} /></div>)}</div></div></section>
      ...
      <section id="booking" className="booking-section section-pad"><div className="booking-aside"><div className="section-kicker">05 / Запис</div><h2>Почнемо<br /><em>з розмови.</em></h2><p>Залиште контакти — адміністратор зателефонує, відповість на запитання та підбере зручний час.</p><div className="booking-contact"><span><Clock3 size={17} /> Пн–Сб, 09:00–20:00</span><span><MessageCircle size={17} /> Viber / Telegram: +380 73 300 77 88</span></div></div><form className="booking-form" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }}><label>Ваше ім'я<input required placeholder="Як до вас звертатися?" /></label><label>Номер телефону<input required type="tel" placeholder="+380 00 000 00 00" /></label><label>Що вас цікавить?<select defaultValue=""><option value="" disabled>Оберіть напрямок</option><option>Консультація</option><option>Терапія</option><option>Імплантація</option><option>Ортодонтія</option><option>Дитяча стоматологія</option></select></label><button className="button button-coral" type="submit">{submitted ? <><Check size={17} /> Дякуємо, ми зателефонуємо</> : <>Записатися на консультацію <ArrowUpRight size={17} /></>}</button><small>Натискаючи кнопку, ви погоджуєтесь на обробку персональних даних.</small></form></section>

      <section id="contacts" className="contacts-section section-pad"><div className="section-kicker">06 / Контакти</div><div className="contacts-grid"><div><h2>Зустрінемося<br /><em>у Рів'єрі.</em></h2><address><span><MapPin size={17} /> м. Миколаїв, вул. Лазурна, 5,<br />корпус 10/1, ЖК Рів'єра, мкр Нимив</span><a href="tel:+380512777888"><Phone size={17} /> +380 512 777 888</a><a href="tel:+380973201527"><Phone size={17} /> +380 97 320 15 27</a></address><div className="contact-links"><a href="https://www.google.com/maps/search/?api=1&query=Миколаїв+Лазурна+5" target="_blank" rel="noreferrer">Відкрити маршрут <ArrowUpRight size={15} /></a><a href="https://instagram.com" target="_blank" rel="noreferrer"><Instagram size={16} /> Instagram</a></div></div><div className="map-card"><div className="map-grid" /><div className="map-pin"><MapPin size={22} fill="currentColor" /><span>Active Medical</span></div><div className="map-label">ЖК Рів'єра<br /><small>Миколаїв</small></div></div></div></section>

      <footer className="footer"><div className="footer-brand"><img src={brandMark} alt="" /><div><b>Актив</b><span>Медікал</span></div></div><p>Стоматологія, в якій<br />вам спокійно.</p><span className="footer-copy">© 2026 Active Medical</span></footer>
    </main>
  );
}
