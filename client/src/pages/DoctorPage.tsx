  ...
  return (
    <main className="doctor-page service-page">
      ...
      <article className="doctor-page-content service-page-content">
        ...
        <div className="doctor-page-hero"><div><h1>{doctor.name}</h1><p className="service-page-lead">{doctor.role}</p><p>{doctor.description}</p><button className="button button-coral" onClick={() => navigate("/#booking")}>Записатись на консультацію <ArrowUpRight size={16} /></button></div><img src={doctor.image} alt={`${doctor.name} — ${doctor.role} в Active Medical`} loading="eager" fetchPriority="high" decoding="async" /></div>
